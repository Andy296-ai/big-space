<script setup lang="ts">
import {
    GitBranch,
    Loader2,
    Plus,
    ScrollText,
    Trash2,
    UserCog,
    UserMinus,
    UserPlus,
    X,
} from 'lucide-vue-next';
import { computed, onMounted, ref } from 'vue';
import { apiFetch } from '../lib/api';
import { fill, useT } from '../lib/i18n';

interface ActivityLogEntry {
    id: number;
    action: string;
    subject_id: number | null;
    meta: Record<string, unknown> | null;
    created_at: string;
    actor: { id: number; name: string; email: string } | null;
}

const props = defineProps<{
    spaceId: number;
}>();

const emit = defineEmits<{
    (e: 'close'): void;
}>();

const t = useT();

const loading = ref(true);
const entries = ref<ActivityLogEntry[]>([]);

const ACTION_ICONS: Record<string, typeof UserPlus> = {
    'node.created': Plus,
    'node.deleted': Trash2,
    'structure.changed': GitBranch,
    'collaborator.added': UserPlus,
    'collaborator.removed': UserMinus,
    'collaborator.role_changed': UserCog,
};

function iconFor(action: string) {
    return ACTION_ICONS[action] ?? ScrollText;
}

/** У каждого типа события своя форма meta — единого шаблона с {name} тут не хватит. */
function describe(entry: ActivityLogEntry): string {
    const meta = entry.meta ?? {};

    switch (entry.action) {
        case 'node.created':
            return fill(t.value.spaceFeedNodeCreated, {
                title:
                    (meta.title as string | undefined) ??
                    `#${entry.subject_id}`,
            });
        case 'node.deleted': {
            const titles = (meta.titles as string[] | undefined) ?? [];
            const count = (meta.count as number | undefined) ?? titles.length;
            const label = titles.length
                ? titles.join(', ')
                : `#${entry.subject_id}`;

            return count > titles.length
                ? fill(t.value.spaceFeedNodeDeletedCascade, {
                      titles: label,
                      count,
                  })
                : fill(t.value.spaceFeedNodeDeleted, { titles: label });
        }
        case 'structure.changed':
            if (meta.action === 'mode_changed') {
                return fill(t.value.spaceFeedStructureMode, {
                    structure: (meta.structure as string | undefined) ?? '',
                });
            }

            return fill(
                meta.action === 'linked'
                    ? t.value.spaceFeedLinked
                    : t.value.spaceFeedUnlinked,
                {
                    parent: (meta.parent_title as string | undefined) ?? '?',
                    child: (meta.child_title as string | undefined) ?? '?',
                },
            );
        case 'collaborator.added':
            return fill(t.value.spaceFeedCollaboratorAdded, {
                name:
                    (meta.name as string | undefined) ?? `#${entry.subject_id}`,
                role: (meta.role as string | undefined) ?? '',
            });
        case 'collaborator.removed':
            return fill(t.value.spaceFeedCollaboratorRemoved, {
                name:
                    (meta.name as string | undefined) ?? `#${entry.subject_id}`,
            });
        case 'collaborator.role_changed':
            return fill(t.value.spaceFeedCollaboratorRoleChanged, {
                name:
                    (meta.name as string | undefined) ?? `#${entry.subject_id}`,
                role: (meta.role as string | undefined) ?? '',
            });
        default:
            return entry.action;
    }
}

function formatTime(iso: string): string {
    return new Date(iso).toLocaleString();
}

const rows = computed(() =>
    entries.value.map((entry) => ({
        entry,
        icon: iconFor(entry.action),
        text: describe(entry),
        actorName: entry.actor?.name ?? t.value.activityLogSystemActor,
        time: formatTime(entry.created_at),
    })),
);

onMounted(async () => {
    try {
        const res = await apiFetch(`/api/spaces/${props.spaceId}/activity-log`);
        const data = await res.json();
        entries.value = data.entries ?? [];
    } catch (err) {
        console.error('Failed to load space activity log:', err);
    } finally {
        loading.value = false;
    }
});
</script>

<template>
    <Teleport to="body">
        <div
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"
            @click.self="emit('close')"
        >
            <div
                class="flex h-[80vh] w-[92vw] max-w-xl flex-col overflow-hidden rounded-3xl border border-slate-700/80 bg-slate-900 text-slate-100 shadow-2xl"
            >
                <div
                    class="flex items-center justify-between gap-3 border-b border-slate-800 px-5 py-3.5"
                >
                    <div class="flex min-w-0 items-center gap-2.5">
                        <div
                            class="rounded-xl border border-amber-500/30 bg-amber-600/20 p-2 text-amber-400"
                        >
                            <ScrollText class="h-4 w-4" />
                        </div>
                        <div class="min-w-0">
                            <h3 class="truncate text-sm font-bold">
                                {{ t.spaceActivityLogTitle }}
                            </h3>
                            <p class="truncate text-[11px] text-slate-400">
                                {{ t.spaceActivityLogDesc }}
                            </p>
                        </div>
                    </div>
                    <button
                        @click="emit('close')"
                        :aria-label="t.close"
                        class="shrink-0 rounded-xl p-1.5 text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-200"
                    >
                        <X class="h-4 w-4" />
                    </button>
                </div>

                <div class="min-h-0 flex-1 overflow-auto p-3">
                    <div
                        v-if="loading"
                        class="flex h-full items-center justify-center text-slate-400"
                    >
                        <Loader2 class="h-5 w-5 animate-spin" />
                    </div>
                    <div
                        v-else-if="!rows.length"
                        class="flex h-full items-center justify-center text-xs text-slate-500"
                    >
                        {{ t.activityLogEmpty }}
                    </div>
                    <div v-else class="space-y-1.5">
                        <div
                            v-for="row in rows"
                            :key="row.entry.id"
                            class="flex items-start gap-3 rounded-2xl border border-slate-800 bg-slate-800/40 p-3"
                        >
                            <div
                                class="mt-0.5 shrink-0 rounded-lg border border-slate-700 bg-slate-800 p-1.5 text-slate-400"
                            >
                                <component :is="row.icon" class="h-3.5 w-3.5" />
                            </div>
                            <div class="min-w-0 flex-1 text-xs">
                                <p class="text-slate-200">
                                    <span
                                        class="font-semibold text-slate-100"
                                        >{{ row.actorName }}</span
                                    >
                                    {{ ' ' }}{{ row.text }}
                                </p>
                                <p class="mt-0.5 text-[10px] text-slate-500">
                                    {{ row.time }}
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </Teleport>
</template>
